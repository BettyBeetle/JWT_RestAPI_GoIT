...
from m12.database.models import Contacts
from m12.schemas import ContactsIn
...
async def search_contacts(search: str, skip: int, limit: int,  user:User, db: Session):
    ...
    contacts = db.query(Contacts).filter(search_query((Contacts.user_id == user.id))).offset(skip).limit(limit).all()
